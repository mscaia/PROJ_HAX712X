...
Pour plus de détails sur les visualisations et les fonctionnalités avancées, consultez la [section 9](#guide-de-lutilisateur).

## 2.3 Configuration des fichiers

Le projet est configuré de manière à utiliser **Quarto** pour la création du site web interactif et **Sphinx** pour tenter une génération automatique de la documentation. Toute la documentation a été initialement rédigée en utilisant des fichiers `.qmd`, en cohérence avec le reste du site. 
...
<details> 
  <summary>`map_trajets(j, h)`</summary>

**Description**: 
Génère une carte représentant tout les trajets moyens empruntés le jour `j` à l'heure `h` représentés par couleur en fonction de l'intensité. 

**Paramètres**:
  <ul>
    <li>`j` (int) : Le numéro du jour de la semaine voulu (0 pour lundi, ..., 6 pour dimanche)
    <li>`h` (int): Le numéro de l'heure dans la journée qu'on veut observer (de 0 à 23)
  <\ul>

**Retourne**:
- (list, str): une liste des stations de départ et d'arrivée de tous les trajets qui n'ont pas été tracés et un message avec le nom de la fonction créée
...
**Description**: 
Crée une liste des données uniquement pour les jours correspondant au jour `j`
...
**Retourne**:
- (list): une liste des données prises un jour `j`
...
**Description**: 
Crée une liste des coordonnées de façon unique pour les jours correspondant au jour `j`
...
**Retourne**:
- (list): une liste des coordonnées actives sur un jour `j`
...
**Description**: 
Compte le nombre de jours correspondant au jour `j`
...
**Retourne**:
-len(D) (int): le nombre de jours étant un jour `j` dans les données
...
**Description**:
Cette fonction donne l'intensité moyenne par coordonnée mesurée sur un jour `j`. 

**Paramètre**:
  <ul>
    <li>`j` (int): Le numéro du jour de la semaine (0 pour lundi, ..., 6 pour dimanche)
  </ul>

**Retourne**:
- `M` (matrix): Une matrice contenant les valeurs des intensités pour chaque coordonnée sur le jour `j` en moyenne. 
...
**Description**:
Cette fonction associe une couleur à une intensité en fonction de sa "position" par rapport aux intensités maximales et minimales 


**Paramètre**:
  <ul>
    <li>`intens` (int): Valeur de l'intensité qu'on cherche à associer à une couleur
    <li>`min_int` (int): Valeur de l'intensité minimale
    <li>`max_int` (int): Valeur de l'intensité maximale 
  </ul>

**Retourne**:
- `rgba(x, y, z, 1) ` (list): Une liste contenant le code couleur associé à l'intensité représentée 
...
<details>
  <summary>`poids_par_heure(j):`</summary>
**Description**:
Cette fonction classe les trajets d'un jour `j` par heure 

**Paramètre**:
- `j` (int): Numéro du jour de la semaine (0 pour lundi, ..., 6 pour dimanche)

**Retourne**:
-`pourcentage de trajets par heure M` avec M (matrix): [[station départ, station arrivée], nombre de trajets *100/nombre total de trajets sur `j`]
...
**Description**:
Cette fonction classe les trajets d'un jour `j` à une heure `h` avec leur multiplicité moyenne. 

**Paramètre**:
  <ul>
    <li>`j` (int): Numéro du jour de la semaine (0 pour lundi, ..., 6 pour dimanche)
    <li>`h` (int): Numéro de l'heure dans la journée (0 pour 00:00-00:59, ..., 23 pour 23:00-23:59]
  </ul>

**Retourne**:
-`F` (matrix): [[station départ, station arrivée], multiplicité à l'heure `h` dans le jour `j`]
...
Les tests réalisés incluent la validation des fonctionnalités suivantes :

- Traitement des données ;
- Gestion des données via des classes spécifiques ;
- Génération de cartes interactives ;
- Prédictions basées sur des algorithmes ;
- Analyse statistique ;
- Visualisation vidéo.

## 7.2 Organisation des tests

Les tests sont organisés dans le dossier `tests/`, qui contient plusieurs fichiers, chacun dédié à un module spécifique du projet :

- `test_donnée.py` : Vérifie les fonctions du fichier `donnée.py` ;
- `test_gestion_donnee.py` : Teste la classe `GestionnaireDonnees` du fichier `gestion_donnee.py` ;
- `test_map.py` : Valide les fonctionnalités de création de cartes dans `map.py` ;
- `test_prediction.py` : Vérifie les algorithmes de prédiction ;
- `test_statistique.py` : Analyse les fonctions statistiques de `statistique.py` ;
- `test_video.py` : Teste les outils de traitement vidéo de `video.py`.
...
- **Objectif** : Valider les fonctions liées au traitement des données brutes.

- **Scénarios principaux** :
  - Chargement et validation des formats de données ;
  - Gestion des erreurs pour des fichiers incorrects ou incomplets.
</details>


### `test_gestion_donnee.py`
- **Objectif** : Tester les méthodes de la classe `GestionnaireDonnees`.
- **Scénarios principaux** :
  - Initialisation avec différents types de données (valides et invalides) ;
  - Filtrage, tri et exportation des données ;
  - Gestion des exceptions.

### `test_map.py`
- **Objectif** : Vérifier les fonctions de création de cartes interactives.
- **Scénarios principaux** :
  - Génération de cartes avec des données correctes ;
  - Validation des couches et éléments interactifs ;
  - Gestion des cas où les données sont manquantes.

(... ajoute des sections similaires pour les autres fichiers testés.)

## 7.4 Résultats des tests [RECHEK]

Tous les modules ont été testés avec succès, et les bugs identifiés ont été corrigés. Une couverture élevée du code a été atteinte grâce à l’utilisation de [outil utilisé, par ex. `pytest-cov`].  
La commande suivante permet d’exécuter tous les tests :

```bash
pytest tests/
```
## 7.5 Intégration continue [RECHEK]

Les tests sont intégrés au processus d’intégration continue (CI) via GitHub Actions. Cela garantit que chaque mise à jour du code est automatiquement testée, minimisant ainsi les risques d’introduire des régressions.
...
cd ./PROJ_HAX712X
```
## 9.2 Installation des packages nécessaire
...
## 9.3 Comment lancer certain script
